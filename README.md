...
| 더쿠 | 게시판 목록 훑기 (검색이 없어 제목으로 거른다, **기본 꺼짐**) | ★★ |
...
**게시판 한정 검색이 `search_keyword`를 무시한다.** 200을 주면서 최신 목록을 그대로 돌려준다
(2026-08 실측: 검색어를 바꿔도, 없는 단어를 넣어도 결과가 목록과 같았다). 통합검색은 403이다.

그래서 게시판을 지정해 **목록을 페이지 단위로 훑고 제목에서 키워드를 거른다.** 설정의
`theqooBoards`에 게시판 이름(URL의 mid 값)을 넣어야 돌고, 비우면 건너뛴다. 업종마다 볼 곳이
달라서 코드에 넣지 않았다.

| 성질 | 내용 |
|---|---|
| 브라우저 | **불필요.** 정적 HTML이라 fetch만 쓴다 (디시, Threads와 다르다) |
| 놓치는 것 | 본문에만 서비스명이 있는 글. 목록에는 제목만 온다 |
| 본문 | 제목이 걸린 글만 추가로 읽는다 (한 번에 최대 30건) |
| 상한 단위 | 건수가 아니라 **쪽수**. 값을 키우면 더 오래된 글까지 내려간다 |

활발한 게시판은 43분에 20건씩 올라온다(실측). 수집 주기가 길면 쪽수를 늘려야 그 사이 글을
다 본다. 실측으로 3쪽(51건)에서 키워드가 걸린 글 15건이 나왔다.


### 다음 카페는 모바일 경로로만 보인다

**데스크톱은 비로그인에게 목록을 주지 않는다.** `bbs_list`가 HTTP 200에 88KB를 돌려주는데
글 링크가 0건이고 `MEMBER_ROLENAME: "비회원"`만 들어 있다(2026-08 실측, 한 카페의 게시판
7개 전부). 모바일(`m.cafe.daum.net/{카페}/{게시판}`)은 같은 게시판을 비로그인으로 내려준다.
로그인 세션이 필요 없어서 X와 달리 계정 리스크가 없다.

목록이 Vue로 렌더되지만 **브라우저는 필요 없다.** 화면을 그리기 전의 데이터가
`articles.push({ dataid, title, writerNickname, articleElapsedTime, ... })` 형태로 HTML에
인라인되어 있어서, 오히려 파싱하기 쉽다. 본문은 글 상세의 `tx-content-container`에서 읽는다.

| 성질 | 내용 |
|---|---|
| 브라우저 | **불필요.** 인라인 데이터를 파싱한다 |
| 상한 | **게시판당 최신 20건.** `?page=2`가 무시되고 스크롤로도 더 안 나온다 |
| 게시판 고르기 | 글이 몰리는 종합 게시판은 20건이 **1~2분 분량**이다(실측: 분당 19건). 주제별 게시판은 같은 20건이 **며칠에서 몇 주**를 덮는다(실측: 11일, 25일) |
| 본문이 막힌 게시판 | 댓글로 대화하는 형식은 글 상세가 403이다. 세 번 겹치면 그 게시판은 제목만 저장한다 |
| 설정 | `daumCafeBoards`에 `카페아이디/게시판아이디`를 넣는다. 비우면 건너뛴다 |

과거 글까지 필요하면 이 경로로는 닿지 않는다. 카카오 검색 API(`/v2/search/cafe`, 일 30,000회
무료)가 키워드 검색과 페이지 넘기기를 지원하지만, 특정 카페만 골라 볼 수는 없다.

### 브리핑 날짜는 작성일 기준이다

**수집일이 아니다.** 앱 리뷰는 오늘 수집해도 작성일이 몇 달 전이라, 수집일로 묶으면 오래된 글이
오늘 여론으로 집계된다. 실측(2026-08-20)에서 그날 브리핑 275건 중 46건이 2025년 글이었다.
첫 수집날에는 11년치가 한 덩어리로 들어간다.

그래서 브리핑, 카테고리 집계, 급증 기준선을 모두 `posted_at`으로 맞췄다. 한쪽만 바꾸면
'그날 작성된 글'과 '직전 며칠에 수집된 글'을 비교하게 되어 급증 판정이 어긋난다.

| 성질 | 내용 |
|---|---|
| 여러 날짜가 갱신된다 | 한 번 수집해도 작성일이 흩어져 있어 그 날짜들의 요약이 낡는다. 파이프라인이 이번 실행에서 들어온 글의 작성일만 골라 다시 만든다 |
| 작성일 없는 글 | 어느 날짜에도 안 들어간다(카페 API가 안 주는 경우 등). 그 건수를 화면과 브리핑에 적는다 |
| 날짜 선택 | 최근 7일은 버튼, 그 앞은 달력. 달력은 `datalist`로 **데이터가 있는 날짜만** 후보로 준다 |

**요약은 글이 많은 날에만 만든다.** 요약은 다 읽을 수 없을 때 쓰는 압축이라, 몇 건뿐인 날에
만들면 원문보다 정보가 줄고 호출만 나간다. `SUMMARY_MIN_ITEMS`(11) 미만인 날짜는 화면이
원문을 그대로 싣는다(스크롤 영역). 실측에서 날짜 1,057개 중 1,033개가 그쪽이라, 전부 요약하면
5,245회였던 호출이 85회로 줄었다.
...
이미 있는 날짜는 건너뛰므로 중간에 멈춰도 다시 돌리면 이어서 한다. 프롬프트를 고쳐 다시
만들려면 `--force`를 붙인다.

### 브리핑은 시간대 집중을 맨 위에 둔다

**급증 감지가 두 가지를 놓쳤다.** 기존 판정은 '직전 7일 평균 대비 3배 + 최소 5건'을
**카테고리별, 하루 단위**로 본다.

- 하루가 끝나야 판정되므로 배포 직후 몇 시간에 몰리는 반응은 당일에 안 보인다
- 같은 사건이 카테고리 셋으로 갈리면 어느 쪽도 임계를 못 넘는다

그래서 시간 축과 주제 묶음을 따로 둔다. 실측(2026-08-20)에서 한 기능 개편 반응이 이렇게 잡혔다.
...
35%여서, '그날의 50% 이상' 같은 문턱을 두면 놓친다. 평소 언급량이 많을수록 비율은 낮게 나온다.

주제어에서는 판정 어휘(`불만`, `요청`)와 검색 키워드를 뺀다. 안 빼면 상위가 그 말들로 채워져
무슨 일인지 알 수 없다. 작성 시각이 없는 글은 이 집계에서 빠지고, 그 건수를 함께 표시한다.
...
다음 카페는 처음에 이 목록에 있었다. 데스크톱 페이지가 로그인 벽으로 보였기 때문인데,
2026-08 재조사에서 **모바일 경로는 비로그인으로 열린다**는 것을 확인해 지원 소스로 옮겼다
(위의 '다음 카페는 모바일 경로로만 보인다' 참고).
