...
```
feedback-radar/
├─ private/                            # 🔒 비공개 파일 전용 (gitignore) — 설정·.env·DB·리포트
│   └─ feedback-radar.config.json      #    다른 머신으로 옮길 땐 이 폴더만 압축
...
요구사항: Node.js **20.12 이상** (`process.loadEnvFile` 사용)
...
cp feedback-radar.config.example.json private/feedback-radar.config.json   # 키워드·앱ID 수정
cp .env.example private/.env                  # 필요한 값만 채우면 됨 (전부 비워도 동작)
...
LLM 모드는 문맥으로 판단하고, 휴리스틱 모드는 설정의 `relevanceHints`(함께 나와야 하는 도메인 단어)로 근사한다.
태거를 바꾼 뒤 기존 데이터를 다시 분류하려면: `npm run retag && npm run collect`
...
비공개 파일(테넌트 설정, `.env`, DB, 리포트, 내부 문서)은 전부 `private/` 한 폴더에 있다.

```bash
# 옮길 때: private/만 압축
tar -czf feedback-radar-private.tgz private/     # 또는 우클릭 → 압축

# 새 머신에서: 클론 후 압축 해제
git clone https://github.com/bedcoding/feedback-radar && cd feedback-radar
