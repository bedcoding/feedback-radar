...
> **한 줄 요약**: 공식 API가 없어 직접 파싱으로만 가져올 수 있는 채널이고, **2026-08-30 결정으로 꺼져 있다.**
...
| 항목 | 값 |
|---|---|
| **현재 상태** | **꺼짐.** 운영 설정(`settings` 표의 `config.sources`)에 `dcinside: true`로 들어 있다 (2026-08-29 DB 실측). 다만 **배포판에서는 강제로 꺼진다** — Vercel 함수에 Chromium이 없다 ([daily.ts:163](../../apps/pipeline/src/daily.ts)) |
