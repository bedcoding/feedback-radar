...
- 짝 문서: [적법성 근거](data-collection-compliance.md) · [전환 계획](official-api-migration.md) · [소스별 연동](sources/)
...
| 항목 | 값 | 출처 |
|---|---|---|
| **키워드 수** | **9개** | ✅ 설정 실측 (서비스 5개에 3+2+1+2+1) |
...
| 대상 | 비용 | 실제 장벽 |
|---|---|---|
| 앱스토어 | **0원 추가** (연회비는 이미 냄) | 판매자 계정 2곳의 키 발급 |
| 구글플레이 | **0원 추가** (등록비는 이미 냄) | 개발자 계정 3곳의 권한 승인 |
| 네이버 | **0원** | 없음 (이미 돌고 있음) |
| Threads | **0원** | **비즈니스 인증 + 앱 심사** |
| X | **월 $1~27** | 결제수단 등록 |
| 카카오 | 0원 | 🔴 [약관 저장 조항](data-collection-compliance.md#4-6-다음-카페--근거-있음) |
