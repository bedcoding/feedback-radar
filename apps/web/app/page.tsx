...
export default async function Home({
  ...
  searchParams: Promise<{ filter?: string; tour?: string; page?: string }>;
}) {
  ...
  const items = getRecentItems(db, PAGE_SIZE, filter, (page - 1) * PAGE_SIZE);
  ...
  return (
    ...
    <DashboardView
      ...
      pager={{
        ...
        // 탭·투어 상태를 유지한 채 쪽만 바꾼다
        href: (p: number) => {
          const q = new URLSearchParams();
          if (filter === 'irrelevant') q.set('filter', 'irrelevant');
