  ...
  return <main className={styles.shell}>
    ...
    {result && <section className={styles.results} aria-label="조회 결과">
      ...
      {!result.videos.length ? <div className={styles.empty}><h3>관련 영상을 찾지 못했습니다</h3><p>서비스의 다른 이름이나 해외 표기로 다시 검색해 보세요.</p></div> : <div className={styles.workspace}>
        ...
        {activeVideo && <article className={styles.reader}><div className={styles.readerHead}><span className={styles.eyebrow}>시청자 댓글</span><h3>{activeVideo.title}</h3><div><span>{activeVideo.status}</span><span>전체 댓글 {activeVideo.commentCount === undefined ? '미확인' : Number(activeVideo.commentCount).toLocaleString() + '개'}</span><span>{activeVideo.summaryNeeded ? '요약 대기' : activeVideo.summarizedAt ? '요약 이후 댓글 수 동일' : '요약 미실행'}</span><a href={activeVideo.url} target="_blank" rel="noreferrer">YouTube에서 보기 ↗</a></div></div>
